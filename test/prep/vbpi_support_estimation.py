...
def get_supports(path, taxa):
    tree_dict_total, tree_names_total, tree_wts_total = get_tree_list_raw(path, hpd=1.0)
    rootsplit_supp_dict, subsplit_supp_dict = get_support_from_samples(
        taxa, tree_dict_total, tree_names_total
    )
    ...
    with open(out_path, "w") as fp:
        fp.write(
            json.dumps(
                {
                    "rootsplit_supp_dict": rootsplit_supp_dict,
                    "subsplit_supp_dict": subsplit_supp_dict,
                }
            )
        )
