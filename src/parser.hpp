...
# if 201103L <= YY_CPLUSPLUS
  ...
  private:
    ...
    TOK_RPAREN = 262,              // ")"
    TOK_LABEL = 263,               // "label"
    TOK_QUOTED = 264,              // "quoted"
    TOK_BRACKETED_WITH_AMPERSAND = 265 // "bracketed_with_ampersand"
      };
    ...
    struct symbol_kind
    {
      enum symbol_kind_type
      {
        ...
        S_LABEL = 8,                             // "label"
        S_QUOTED = 9,                            // "quoted"
        S_BRACKETED_WITH_AMPERSAND = 10,         // "bracketed_with_ampersand"
        S_YYACCEPT = 11,                         // $accept
        S_tree = 12,                             // tree
        S_fancy_node = 13,                       // fancy_node
        S_node = 14,                             // node
        S_leaf = 15,                             // leaf
        S_inner_node = 16,                       // inner_node
        S_node_list = 17,                        // node_list
        S_metadata_comment_option = 18           // metadata_comment_option
      };
    };
...
switch (yykind)
    ...
    struct symbol_type : basic_symbol<by_kind>
    {
      ...
      {
#if !defined _MSC_VER || defined __clang__
        YY_ASSERT ((token::TOK_LABEL <= tok && tok <= token::TOK_BRACKETED_WITH_AMPERSAND));
#endif
      }
    };
    ...
    // Implementation of make_symbol for each token kind.
      ...
      static
      symbol_type
      make_QUOTED (std::string v, location_type l)
      {
        return symbol_type (token::TOK_QUOTED, std::move (v), std::move (l));
      }
#else
      static
      symbol_type
      make_QUOTED (const std::string& v, const location_type& l)
      {
      ...
      static
      symbol_type
      make_BRACKETED_WITH_AMPERSAND (std::string v, location_type l)
      {
        return symbol_type (token::TOK_BRACKETED_WITH_AMPERSAND, std::move (v), std::move (l));
      }
#else
      static
      symbol_type
      make_BRACKETED_WITH_AMPERSAND (const std::string& v, const location_type& l)
      {
        return symbol_type (token::TOK_BRACKETED_WITH_AMPERSAND, v, l);
      }
#endif
