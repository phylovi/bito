...
// This operation sets the PLV for op.dest_ to zero and the rescaling amount for the PLV
// in op.dest_ to be the minimum of that for all of the PLVs in op.src_vector_. We do
// this so that we can sum over partial likelihood vectors after rescaling each one so
// that it is on the same scale as dest. Note that we want the minimum here because we
// want to preserve accuracy for the PLV with the highest likelihood (which corresponds
// to the least amount of rescaling).
