...
void RootedTree::InitializeParameters(
  ...
  height_ratios_ = EigenVectorXd(leaf_count - 1, -1);
  node_heights_ = EigenVectorXd(Topology()->Id() + 1);
  node_bounds_ = EigenVectorXd(Topology()->Id() + 1);
  rates_ = EigenVectorXd(Topology()->Id(), 1.0);
...
TagDoubleMap RootedTree::TagDateMapOfDateVector(EigenVectorXd leaf_date_vector) {
...
RootedTree RootedTree::Example() {
  ...
  EigenVectorXd date_vector({5., 3., 0., 1.});
