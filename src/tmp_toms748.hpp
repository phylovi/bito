...
inline T safe_div(T num, T denom, T r) {
  ...
  BOOST_MATH_STD_USING  // For ADL of std math functions
      ...
      if (fabs(denom) < 1) {
    if (fabs(denom * tools::max_value<T>()) <= fabs(num)) return r;
  }
  return num / denom;
}
...
inline T secant_interpolate(const T& a, const T& b, const T& fa, const T& fb) {
  ...
  T c = a - (fa / (fb - fa)) * (b - a);
  if ((c <= a + fabs(a) * tol) || (c >= b - fabs(b) * tol)) return (a + b) / 2;
  return c;
}

template <class T>
T quadratic_interpolate(const T& a, const T& b, T const& d, const T& fa, const T& fb,
                        T const& fd, unsigned count) {
  //
  ...
  for (unsigned i = 1; i <= count; ++i) {
    ...
    c -= safe_div(T(fa + (B + A * (c - b)) * (c - a)), T(B + A * (2 * c - a - b)),
                  T(1 + c - a));
  }
  if ((c <= a) || (c >= b)) {
...
T cubic_interpolate(const T& a, const T& b, const T& d, const T& e, const T& fa,
  ...
  //
  BOOST_MATH_INSTRUMENT_CODE(" a = " << a << " b = " << b << " d = " << d
                                     << " e = " << e << " fa = " << fa << " fb = " << fb
                                     << " fd = " << fd << " fe = " << fe);
  T q11 = (d - e) * fd / (fe - fd);
  T q21 = (b - d) * fb / (fd - fb);
  ...
  T d21 = (b - d) * fd / (fd - fb);
  T d31 = (a - b) * fb / (fb - fa);
  BOOST_MATH_INSTRUMENT_CODE("q11 = " << q11 << " q21 = " << q21 << " q31 = " << q31
                                      << " d21 = " << d21 << " d31 = " << d31);
  ...
  T c = q31 + q32 + q33 + a;
  BOOST_MATH_INSTRUMENT_CODE("q22 = " << q22 << " q32 = " << q32 << " d32 = " << d32
                                      << " q33 = " << q33 << " c = " << c);
  ...
  if ((c <= a) || (c >= b)) {
    ...
    BOOST_MATH_INSTRUMENT_CODE(
        "Out of bounds interpolation, falling back to quadratic interpolation. c = "
        << c);
  }
...
std::pair<T, T> toms748_solve(F f, const T& ax, const T& bx, const T& fax, const T& fbx,
                              Tol tol, boost::uintmax_t& max_iter, const Policy& pol) {
  //
  ...
  b = bx;
  if (a >= b)
    policies::raise_domain_error(function, "Parameters a and b out of order: a=%1%", a,
                                 pol);
  fa = fax;
  fb = fbx;
  ...
  if (boost::math::sign(fa) * boost::math::sign(fb) > 0)
    policies::raise_domain_error(
        function, "Parameters a and b do not bracket the root: a=%1%", a, pol);
...
inline std::pair<T, T> toms748_solve(F f, const T& ax, const T& bx, const T& fax,
                                     const T& fbx, Tol tol,
                                     boost::uintmax_t& max_iter) {
  return toms748_solve(f, ax, bx, fax, fbx, tol, max_iter, policies::policy<>());
}

// fax, fbx not computed.
template <class F, class T, class Tol, class Policy>
inline std::pair<T, T> toms748_solve(F f, const T& ax, const T& bx, Tol tol,
                                     boost::uintmax_t& max_iter, const Policy& pol) {
  max_iter -= 2;
...
// no policy and fax, fbx not computed.
template <class F, class T, class Tol>
inline std::pair<T, T> toms748_solve(F f, const T& ax, const T& bx, Tol tol,
                                     boost::uintmax_t& max_iter) {
...
template <class F, class T, class Tol, class Policy>
std::pair<T, T> bracket_and_solve_root(F f, const T& guess, T factor, bool rising,
  ...
  if ((fa < 0) == (guess < 0 ? !rising : rising)) {
    ...
    while ((boost::math::sign)(fb) == (boost::math::sign)(fa)) {
      ...
      fb = f(b);
      --count;
      BOOST_MATH_INSTRUMENT_CODE("a = " << a << " b = " << b << " fa = " << fa
                                        << " fb = " << fb << " count = " << count);
    }
  } else {
    //
    ...
    while ((boost::math::sign)(fb) == (boost::math::sign)(fa)) {
      ...
      if (count == 0)
        policies::raise_evaluation_error(
            function, "Unable to bracket root, last nearest value was %1%", a, pol);
      //
      // Heuristic: every 20 iterations we double the growth factor in case the
      // initial guess was *really* bad !
      //
...
template <class F, class T, class Tol>
inline std::pair<T, T> bracket_and_solve_root(F f, const T& guess, const T& factor,
                                              bool rising, Tol tol,
                                              boost::uintmax_t& max_iter) {
  return bracket_and_solve_root(f, guess, factor, rising, tol, max_iter,
                                policies::policy<>());
}
