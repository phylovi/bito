...
std::vector<double> BifurcatingTreeBranchGradients(
  ...
  tree->ForceZeroBranchLength();

  std::vector<int> node_indices(branch_count);
  std::iota(node_indices.begin(), node_indices.end(), 0);

  std::vector<int> derv_indices(branch_count);
  std::iota(derv_indices.begin(), derv_indices.end(), branch_count);
  ...
  tree->Topology()->TripleIndexPreOrderBifurcating(
      [&](int parent_index, int sister_index, int node_index) {
        ...
        if (node_index != root_child_index && node_index != fixed_node_index) {
          int upper_partial_index = parent_index + branch_count;
          int upper_matrix_index = parent_index;
          ...
          BeagleOperation op = {node_index + branch_count,
                                BEAGLE_OP_NONE,
                                BEAGLE_OP_NONE,
                                upper_partial_index,
                                upper_matrix_index,
  ...
  beagleUpdateTransitionMatrices(beagle_instance,
                                 0,  // eigenIndex
                                 node_indices.data(),
                                 derv_indices.data(),  // firstDerivativeIndices
                                 NULL,                 // secondDervativeIndices
                                 tree->BranchLengths().data(), branch_count);
  beagleUpdatePartials(beagle_instance,
                       operations.data(),  // eigenIndex
                       static_cast<int>(operations.size()),
                       BEAGLE_OP_NONE);  // cumulative scale index
  ...
  std::vector<int> node_indexes = {0};
  std::vector<int> node_mat_indexes = {0};
  ...
  tree->Topology()->TripleIndexPreOrderBifurcating(
      [&](int parent_index, int sister_index, int node_index) {
        if (node_index != fixed_node_index) {
          double dlogLp;
          double log_like = 0;
          upper_partials_index[0] = node_index + branch_count;
          node_indexes[0] = node_index;
          node_mat_indexes[0] = node_index;
          node_deriv_index[0] = node_index + branch_count;
          ...
          if (node_indexes[0] > upper_partials_index[0]) {
            int temp = node_indexes[0];
            node_indexes[0] = upper_partials_index[0];
            upper_partials_index[0] = temp;
          }
          beagleCalculateEdgeLogLikelihoods(
              beagle_instance,              // instance number
              upper_partials_index.data(),  // indices of parent partialsBuffers
              node_indexes.data(),          // indices of child partialsBuffers
              node_mat_indexes.data(),      // transition probability matrices
