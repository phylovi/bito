...
PCSPDict SBNMaps::PCSPCounterOf(const Node::TopologyCounter& topologies) {
  PCSPDict pcsp_dict;
  for (const auto& iter : topologies) {
    auto topology = iter.first;
    auto count = iter.second;
    ...
    Assert(topology->Children().size() == 3,
           "PCSPCounterOf was expecting a tree with a trifurcation at the root!");
    topology->PCSPPreOrder([&pcsp_dict, &count, &leaf_count](
      ...
      if (search == pcsp_dict.end()) {
        ...
        SafeInsert(pcsp_dict, std::move(parent), child_singleton);
      } else {
...
IndexerRepresentation SBNMaps::IndexerRepresentationOf(const BitsetSizeMap& indexer,
  ...
  topology->PCSPPreOrder([&indexer, &default_index, &leaf_count, &result, &topology](
                             const Node* sister_node, bool sister_direction,
                             const Node* focal_node, bool focal_direction,
                             const Node* child0_node, bool child0_direction,
                             const Node* child1_node, bool child1_direction,
                             const Node* virtual_root_clade) {
    const auto bitset = PCSPBitsetOf(leaf_count, sister_node, sister_direction,
