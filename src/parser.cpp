...
#line 375 "src/parser.cpp"
        break;

      case 9: // "quoted"
#line 59 "src/parser.yy"
        { yyo << yysym.value.template as < std::string > (); }
#line 381 "src/parser.cpp"
        break;
...
#line 66 "src/parser.yy"
    {
    auto t = std::make_shared<Tree>(yystack_[1].value.as < Node::NodePtr > (), drv.branch_lengths_);
...
#line 76 "src/parser.yy"
  ...
  try {
      assert(drv.branch_lengths_.insert(
        std::pair<int64_t, double>(yystack_[2].value.as < Node::NodePtr > ()->Tag(), std::stof(yystack_[0].value.as < std::string > ()))).second);
  } catch (...) {
...
#undef YYCASE_
  ...
  "\"(\"", "\")\"", "\"label\"", "\"quoted\"", "$accept", "tree",
  "fancy_node", "node", "leaf", "inner_node", "node_list", YY_NULLPTR
  };
  ...
  parser::yyrline_[] =
  {
       0,    66,    66,    75,    76,    89,   107,   110,   113,   118,
     125,   129
  };
