...
TEST_CASE("GPInstance: hybrid marginal only uncertainty above 5 taxa") {
  ...
  // https://github.com/phylovi/libsbn/issues/323#issuecomment-783349464
  auto inst = GPInstanceOfFiles(
      fasta_path, "data/5-taxon-hybrid-marginal-only-rootward-uncertainty.nwk");
  ...
  const size_t parent_id = 6;
  const size_t child_id = 5;
...
// (0,(1,(2,(3,4))));
// (1,(0,(2,(3,4))));
TEST_CASE("GPInstance: hybrid marginal only uncertainty above 5 taxa; alternate") {
  const std::string fasta_path = "data/5-taxon-slice-of-ds1.fasta";
  // See the DAG at
  // https://github.com/phylovi/libsbn/issues/323#issuecomment-787442591
  auto inst = GPInstanceOfFiles(
