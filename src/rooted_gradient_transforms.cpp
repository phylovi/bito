...
EigenVectorXd HeightGradient(const RootedTree &tree,
  ...
  EigenVectorXd height_gradient(tree.LeafCount() - 1, 0);
...
EigenVectorXd GetLogTimeArray(const RootedTree &tree) {
  ...
  EigenVectorXd log_time(leaf_count - 1, 0);
...
EigenVectorXd UpdateGradientUnWeightedLogDensity(
    const RootedTree &tree, EigenConstVectorXdRef gradient_height) {
  size_t leaf_count = tree.LeafCount();
  size_t root_id = tree.Topology()->Id();
  EigenVectorXd ratiosGradientUnweightedLogDensity(leaf_count - 1);
...
double UpdateHeightParameterGradientUnweightedLogDensity(
  ...
  EigenVectorXd multiplierArray(leaf_count - 1);
...
EigenVectorXd RatioGradientOfHeightGradient(const RootedTree &tree,
  ...
  EigenVectorXd gradientLogDensity =
  ...
  EigenVectorXd log_time = GetLogTimeArray(tree);

  EigenVectorXd gradientLogJacobianDeterminant =
...
EigenVectorXd RatioGradientOfBranchGradient(const RootedTree &tree,
  ...
  EigenVectorXd height_gradient = HeightGradient(tree, branch_gradient);
