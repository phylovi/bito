...
int SubsplitDAG::Compare(const SubsplitDAG &other, const bool quiet) {
...
int SubsplitDAG::Compare(const SubsplitDAG &lhs, const SubsplitDAG &rhs,
  ...
  if (taxon_diff != 0) {
    if (!quiet) {
      std::cout << "SubsplitDAGs have different taxon counts." << std::endl;
    }
    return taxon_diff;
  }
  ...
  if (lhs_nodes != rhs_nodes) {
    if (!quiet) {
      std::cout << "SubsplitDAGs have different node bitsets." << std::endl;
    }
  ...
  if (lhs_edges != rhs_edges) {
    if (!quiet) {
      std::cout << "SubsplitDAGs have different edge bitsets." << std::endl;
    }
...
size_t SubsplitDAG::EdgeCount() const {
  return EdgeCountWithLeafSubsplits() - TaxonCount();
}
...
SubsplitDAGNode SubsplitDAG::GetDAGNode(const Bitset &node_subsplit) const {
  Assert(ContainsNode(node_subsplit),
         "Node with the given node_subsplit does not exist in DAG.");
  const auto node_id = GetDAGNodeId(node_subsplit);
  return GetDAGNode(node_id);
}

MutableSubsplitDAGNode SubsplitDAG::GetDAGNode(const Bitset &node_subsplit) {
  Assert(ContainsNode(node_subsplit),
         "Node with the given node_subsplit does not exist in DAG.");
  const auto node_id = GetDAGNodeId(node_subsplit);
  return GetDAGNode(node_id);
...
size_t SubsplitDAG::GetEdgeIdx(const size_t parent_id, const size_t child_id) const {
  auto edge = storage_.GetLine(parent_id, child_id);
  if (!edge.has_value()) Failwith("Edge not found");
  return edge.value().GetId();
}

size_t SubsplitDAG::GetEdgeIdx(const Bitset &edge_pcsp) const {
...
const BitsetSizePairMap &SubsplitDAG::GetParentNodeToChildEdgeRangeMap() const {
...
size_t SubsplitDAG::CreateAndInsertNode(const Bitset &subsplit,
                                        std::optional<ModificationResult> opt_mod) {
...
size_t SubsplitDAG::CreateAndInsertEdge(const size_t parent_id, const size_t child_id,
  ...
  storage_.AddLine({edge_idx, parent_id, child_id,
                    rotated ? SubsplitClade::Left : SubsplitClade::Right});
  return edge_idx;
}
...
void SubsplitDAG::ConnectNodeToAllDirectedNeighbors(
    const Bitset &node_subsplit, SizeVector &added_edge_idxs,
    std::optional<const Bitset> ignored_subsplit) {
  const auto node_id = GetDAGNodeId(node_subsplit);
  const auto &[neighbors_left, neighbors_right] =
      (direction == Direction::Rootward) ? BuildParentIdVectors(node_subsplit)
                                         : BuildChildIdVectors(node_subsplit);
  for (const auto &focal_clade : {SubsplitClade::Left, SubsplitClade::Right}) {
    const auto &neighbors =
        (focal_clade == SubsplitClade::Left) ? neighbors_left : neighbors_right;
    // #350 Need to phase out rotated.
    const bool is_rotated = (focal_clade == SubsplitClade::Left);
    // If node's children are being added, update parent-to-child-range map.
    if (direction == Direction::Leafward) {
      SafeInsert(parent_to_child_range_,
                 SubsplitToSortedOrder(node_subsplit, is_rotated),
                 {EdgeCountWithLeafSubsplits(),
                  EdgeCountWithLeafSubsplits() + neighbors.size()});
    }
    for (const auto &adj_node_id : neighbors) {
      if (ignored_subsplit.has_value() &&
          (GetDAGNode(adj_node_id).GetBitset() == ignored_subsplit.value())) {
        continue;
      }
      ...
      const auto new_edge_idx =
          CreateAndInsertEdge(parent_node_id, child_node_id, is_rotated);
      added_edge_idxs.push_back(new_edge_idx);
    }
  }
}

SubsplitDAG::ModificationResult SubsplitDAG::AddNodePair(
    const NNIOperation &nni, std::optional<ModificationResult> opt_mods) {
  return AddNodePair(nni.parent_, nni.child_, opt_mods);
}
...
SubsplitDAG::ModificationResult SubsplitDAG::AddNodePair(
  ...
  if (child_is_new) {
    auto new_child_id = CreateAndInsertNode(child_subsplit);
    mods.added_node_ids.push_back(GetDAGNodeId(child_subsplit));

    // Don't reindex these edges.
    ConnectNodeToAllDirectedNeighbors<Direction::Leafward>(child_subsplit,
                                                           mods.added_edge_idxs);
  }
  // If parent node is new, add node it to all its children (except new_child).
  if (parent_is_new) {
    auto new_parent_id = CreateAndInsertNode(parent_subsplit);
    mods.added_node_ids.push_back(GetDAGNodeId(parent_subsplit));
    // Don't reindex these edges.
    ConnectNodeToAllDirectedNeighbors<Direction::Leafward>(
        parent_subsplit, mods.added_edge_idxs, child_subsplit);
  }
  ...
  mods.added_edge_idxs.push_back(EdgeCountWithLeafSubsplits());
  ...
  if (child_is_new) {
    // Reindex these edges.
    ConnectNodeToAllDirectedNeighbors<Direction::Rootward>(
        child_subsplit, mods.added_edge_idxs, parent_subsplit);
  }
  if (parent_is_new) {
    // Reindex these edges.
    ConnectNodeToAllDirectedNeighbors<Direction::Rootward>(parent_subsplit,
                                                           mods.added_edge_idxs);
  }

  if (!ContainsGraft()) {
    // Create reindexers.
    Reindexer node_reindexer = BuildNodeReindexer(prev_node_count);
    mods.node_reindexer = node_reindexer;
    Reindexer edge_reindexer = BuildEdgeReindexer(prev_edge_count);
    mods.edge_reindexer = edge_reindexer;
    // Update the ids in added_node_ids and added_edge_idxs according to the reindexers.
    Reindexer::RemapIdVector(mods.added_node_ids, node_reindexer);
    Reindexer::RemapIdVector(mods.added_edge_idxs, edge_reindexer);
  ...
  // std::cout << "EDGES_NODEPAIR: " << (EdgeCountWithLeafSubsplits() - prev_edge_count)
  //           << std::endl;
  // for (size_t i = 0; i < EdgeCountWithLeafSubsplits(); i++) {
  //   const auto parent_id = GetDAGEdge(i).GetParent();
  //   const auto child_id = GetDAGEdge(i).GetChild();
  //   std::cout << i << ":{" << parent_id << "," << child_id << "}, ";
  //   if (parent_id < child_id) {
  //     std::cout << " <XXX> -> " << (!ContainsGraft() ? "NOT_GRAFT" : "GRAFT")
  //               << std::endl;
  //   }
  // }
  // std::cout << std::endl;
...
using NodeArgsort =
    ArgsortVector<DAGVertex, const DAGVertex &, VerticesView, VerticesView>;
using EdgeArgsort = ArgsortVector<LineView, ConstLineView, LinesView, ConstLinesView>;

std::vector<DAGVertex> ArgsortTestSortedDataVector(const NodeArgsort &argsort_vector) {
  std::vector<DAGVertex> sorted_vector(argsort_vector.Size());
  std::vector<DAGVertex> unsorted_vector(argsort_vector.Size());
  BoolVector sorted_order, unsorted_order;
  for (size_t idx = 0; idx < argsort_vector.Size(); idx++) {
    const auto &unsorted_idx = argsort_vector.GetUnsortedIndexBySortedIndex(idx);
    const auto &sorted_idx = argsort_vector.GetSortedIndexByUnsortedIndex(idx);
    std::cout << "unsorted_idx: " << idx << " -> " << unsorted_idx << std::endl;
    std::cout << "sorted_idx: " << idx << " -> " << sorted_idx << std::endl;
    sorted_vector[idx] = argsort_vector.GetDataBySortedIndex(idx);
    unsorted_vector[idx] = argsort_vector.GetDataByUnsortedIndex(idx);
  }
  for (size_t idx = 1; idx < argsort_vector.Size(); idx++) {
    sorted_order.push_back(Bitset::SubsplitCompare(sorted_vector[idx - 1].GetSubsplit(),
                                                   sorted_vector[idx].GetSubsplit()) <=
                           0);
    unsorted_order.push_back(
        Bitset::SubsplitCompare(unsorted_vector[idx - 1].GetSubsplit(),
                                unsorted_vector[idx].GetSubsplit()) <= 0);
  }
  std::cout << "sorted_order: " << sorted_order << std::endl;
  std::cout << "unsorted_order: " << unsorted_order << std::endl;
  return sorted_vector;
}

SubsplitDAG::ModificationResult SubsplitDAG::AddNodes(
    BitsetVector &node_subsplits, const bool enforce_validity,
    std::optional<ModificationResult> opt_mods) {
  Assert(!enforce_validity || IsValidAddNodes(node_subsplits),
         "Adding given nodes would result in an invalid DAG.");

  const size_t prev_node_count = NodeCount();
  const size_t prev_edge_count = EdgeCountWithLeafSubsplits();
  ...
  auto lessthan_vertices = [](const DAGVertex &lhs, const DAGVertex &rhs) -> bool {
    return Bitset::SubsplitCompare(lhs.GetSubsplit(), rhs.GetSubsplit()) < 0;
  };
  auto access_vertex = [this](VerticesView data_vector,
                              const size_t i) -> const DAGVertex & {
    return storage_.GetVertex(i);
  };
  auto append_vertex = [this](VerticesView data_vector, const DAGVertex &data) -> void {
    storage_.AddVertex(data);
  };
  auto reindex_vertices = [this](VerticesView data_vector,
                                 const Reindexer &reindexer) -> void {
    RemapNodeIds(reindexer);
  };
  auto node_argsort =
      ArgsortVector<DAGVertex, const DAGVertex &, VerticesView, VerticesView>(
          storage_.GetVertices(), Reindexer::IdentityReindexer(NodeCount()),
          lessthan_vertices, access_vertex, append_vertex, reindex_vertices, true);

  node_argsort.SortReindexer();
  ArgsortTestSortedDataVector(node_argsort);
  ...
  for (const auto &node_subsplit : node_subsplits) {
    vertices.push_back({NoId, node_subsplit});
  }

  for (size_t i = 0; i < node_subsplits.size(); i++) {
    const auto &node_subsplit = node_subsplits[i];
    // Add node if it is not already in the DAG.
    if (!ContainsNode(node_subsplit)) {
      // const auto node_id = CreateAndInsertNode(node_subsplit);
      const auto node_id = NodeCount();
      const DAGVertex dag_vertex = DAGVertex(node_id, node_subsplit);
      const auto sorted_idx = node_argsort.FindUniqueSortedIndex(dag_vertex);
      node_argsort.SortedInsert({dag_vertex});
      mods.added_node_ids.push_back(node_id);
      std::cout << "[NODE_INSERT]: " << node_id << " " << sorted_idx << std::endl;
    }
  }

  // // Reindex if SubsplitDAG does not have a graft.
  // if (!ContainsGraft()) {
  // Create reindexers.
  //   Reindexer node_reindexer = BuildNodeReindexer(prev_node_count);
  //   mods.node_reindexer = node_reindexer;
  ...
  // auto lessthan_edges = [](ConstLineView lhs, ConstLineView rhs) -> bool {
  //   if (lhs.GetParent() != rhs.GetParent()) {
  //     return lhs.GetParent() - rhs.GetParent() < 0;
  //   }
  //   return lhs.GetChild() - lhs.GetChild() < 0;
  // };
  // auto access_edge = [this](LinesView data_vector, const size_t i) -> ConstLineView {
  //   return storage_.GetLine(i).value();
  // };
  // auto append_edge = [this](LinesView data_vector, ConstLineView data) -> void {
  //   const auto &id = data.GetId();
  //   const auto &parent_id = data.GetParent();
  //   const auto &child_id = data.GetChild();
  //   const auto &clade = data.GetSubsplitClade();
  //   storage_.AddLine({id, parent_id, child_id, clade});
  // };
  // auto reindex_edges = [this](LinesView data_vector,
  //                             const Reindexer &reindexer) -> void {
  //   RemapEdgeIdxs(reindexer);
  // };
  // auto edge_argsort = ArgsortVector<LineView, ConstLineView, LinesView, LinesView>(
  //     storage_.GetLines(),
  //     Reindexer::IdentityReindexer(EdgeCountWithLeafSubsplits()), lessthan_edges,
  //     access_edge, reindex_edges);

  // // for (size_t i = 0; i < node_subsplits.size(); i++) {
  // //   const auto &node_subsplit = node_subsplits[i];
  // //   // Connect all rootward and leafward edges.
  // //   std::cout << "[CONNECT_0]: " << EdgeCountWithLeafSubsplits() << std::endl;
  // //   ConnectNodeToAllDirectedNeighbors<Direction::Rootward>(node_subsplit,
  // //                                                          mods.added_edge_idxs);
  // //   std::cout << "[CONNECT_1]: " << EdgeCountWithLeafSubsplits() << std::endl;
  // //   ConnectNodeToAllDirectedNeighbors<Direction::Leafward>(node_subsplit,
  // //                                                          mods.added_edge_idxs);
  // //   std::cout << "[CONNECT_2]: " << EdgeCountWithLeafSubsplits() << std::endl;
  // // }

  // // // If SubsplitDAG does not have a graft.
  // // if (!ContainsGraft()) {
  // //   // Create reindexers.
  // //   Reindexer node_reindexer = BuildNodeReindexer(prev_node_count);
  // //   mods.node_reindexer = node_reindexer;
  ...
  // //   mods.edge_reindexer = edge_reindexer;
  // //   // // Update the ids in added_node_ids and added_edge_idxs according to the
  // //   // reindexers. Reindexer::RemapIdVector(mods.added_node_ids, node_reindexer);
  // //   // Reindexer::RemapIdVector(mods.added_edge_idxs, edge_reindexer);
  // //   // // Update fields in the Subsplit DAG according to the reindexers.
  // //   // RemapNodeIds(node_reindexer);
  // //   // RemapEdgeIdxs(edge_reindexer);
  ...
  // //   // CountTopologies();
  // // }
  // // std::cout << "[HOST_1]" << std::endl;

  // std::cout << "EDGES_NODES: " << (EdgeCountWithLeafSubsplits() - prev_edge_count)
  //           << std::endl;
  // for (size_t i = 0; i < EdgeCountWithLeafSubsplits(); i++) {
  //   const auto parent_id = GetDAGEdge(i).GetParent();
  //   const auto child_id = GetDAGEdge(i).GetChild();
...
SubsplitDAG::ModificationResult SubsplitDAG::RemoveNodes(
    const SizeVector &node_ids, const bool enforce_validity,
    std::optional<ModificationResult> opt_mods) {
  // Assert(!enforce_validity || IsValidAddNodes(node_ids),
  //        "Removing given nodes would result in an invalid DAG.");

  const SizeVector &removed_node_ids = node_ids;
  SizeVector removed_edge_idxs;
...
SubsplitDAG::ModificationResult SubsplitDAG::RemoveNodes(
    const BitsetVector &node_subsplits, const bool enforce_validity,
    std::optional<ModificationResult> opt_mods) {
  SizeVector node_ids;
  for (const auto &subsplit : node_subsplits) {
    const auto node_id = GetDAGNodeId(subsplit);
    node_ids.push_back(node_id);
  }
  return RemoveNodes(node_ids, enforce_validity);
}

SubsplitDAG::ModificationResult SubsplitDAG::FullyConnect() {
  ModificationResult mods(NodeCount(), EdgeCountWithLeafSubsplits(), false);
    ...
    for (const auto &children : {left_children, right_children}) {
      ...
      for (const auto child_id : children) {
        if (!ContainsEdge(node.Id(), child_id)) {
          ...
          mods.added_edge_idxs.push_back(edge_idx);
        }
      }
    }
  }
  // Create reindexer and update fields.
  mods.edge_reindexer = BuildEdgeReindexer(prv_edge_count);
  Reindexer::RemapIdVector(mods.added_edge_idxs, mods.edge_reindexer);
...
bool SubsplitDAG::IsConsistent() const {
  // Check that Node's subsplit match subsplit found in map.
  for (const auto &[node_subsplit, node_id] : GetSubsplitToIdMap()) {
    const auto &test_subsplit = GetDAGNode(node_id).GetBitset();
    if (test_subsplit != node_subsplit) {
      return false;
    }
  }
  // Check that every node in storage is also in map.
  if (GetSubsplitToIdMap().size() != storage_.GetVertices().size()) {
    return false;
  }
  // Check that edge range map matches the edge storage vector.
  size_t total_edge_map_count = 0;
  for (const auto &[focal_bitset, child_edge_range] :
       GetParentNodeToChildEdgeRangeMap()) {
    const auto parent_bitset =
        Bitset::Subsplit(focal_bitset.SubsplitGetClade(SubsplitClade::Left),
                         focal_bitset.SubsplitGetClade(SubsplitClade::Right));
    if (!ContainsNode(parent_bitset)) {
      return false;
    }
    const auto &parent_id = GetDAGNodeId(parent_bitset);
    const auto &[edge_idx_begin, edge_idx_end] = child_edge_range;

    total_edge_map_count += (edge_idx_end - edge_idx_begin);
    for (size_t idx = edge_idx_begin; idx < edge_idx_end; idx++) {
      if (parent_id != GetDAGEdge(idx).GetParent()) {
        return false;
      }
      size_t child_id = GetDAGEdge(idx).GetChild();
      if (!ContainsEdge(parent_id, child_id)) {
        return false;
      }
    }
  }
  // Check that every edge (minus leaves) in edge storage vector is in map.
  if (total_edge_map_count - RootsplitCount() != EdgeCount()) {
    return false;
  }
  return true;
}
...
bool SubsplitDAG::IsValid() const {
  ...
  for (auto node : storage_.GetVertices()) {
    // Check that internal node id matches it's position in node array.
...
bool SubsplitDAG::IsTopologicallySorted() const {
  for (const auto &node : storage_.GetVertices()) {
  ...
  DepthFirstWithAction({dag_root_node_id},
                       SubsplitDAGTraversalAction(
                           ...
                           [&node_reindexer, &running_traversal_idx](size_t node_id) {
                             node_reindexer.SetReindex(node_id, running_traversal_idx);
