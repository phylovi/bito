...
void GPEngine::operator()(const GPOperations::IncrementWithWeightedEvolvedPLV& op) {
  ...
  // worthwhile.
  GetPLV(op.dest_) +=
      rescaling_factor * q_(op.gpcsp_) * transition_matrix_ * GetPLV(op.src_);
}
...
void GPEngine::operator()(const GPOperations::IncrementMarginalLikelihood& op) {
  ...
  log_likelihoods_.row(op.rootsplit_) =
      (GetPLV(op.stationary_times_prior_).transpose() * GetPLV(op.p_))
          .diagonal()
          .array()
          .log() +
...
void GPEngine::operator()(const GPOperations::Multiply& op) {
  GetPLV(op.dest_).array() = GetPLV(op.src1_).array() * GetPLV(op.src2_).array();
...
void GPEngine::CopyPLVData(const size_t src_plv_idx, const size_t dest_plv_idx) {
  ...
  GetPLV(dest_plv_idx) = GetPLV(src_plv_idx);
...
void GPEngine::HotStartBranchLengths(const RootedTreeCollection& tree_collection,
  ...
  auto tally_branch_lengths_and_gpcsp_count =
      [&observed_gpcsp_counts, this](EdgeId gpcsp_idx, const RootedTree& tree,
        ...
        size_t gpcsp_id = size_t(gpcsp_idx);
  ...
  for (EdgeId gpcsp_idx = EdgeId(0); gpcsp_idx < unique_gpcsp_count; gpcsp_idx++) {
    size_t gpcsp_id = size_t(gpcsp_idx);
    if (observed_gpcsp_counts(gpcsp_id) == 0) {
      branch_lengths_(gpcsp_id) = default_branch_length_;
    } else {
      // Normalize the branch length total using the counts to get a mean branch
      // length.
      branch_lengths_(gpcsp_id) /= static_cast<double>(observed_gpcsp_counts(gpcsp_id));
    }
  }
}
...
SizeDoubleVectorMap GPEngine::GatherBranchLengths(
  ...
  auto gather_branch_lengths = [&gpcsp_branchlengths_map](EdgeId gpcsp_idx,
                                                          ...
                                                          const Node* focal_node) {
    size_t gpcsp_id = size_t(gpcsp_idx);
    gpcsp_branchlengths_map[gpcsp_id].push_back(tree.BranchLength(focal_node));
  };
...
EigenVectorXd GPEngine::CalculateQuartetHybridLikelihoods(
  ...
  for (const auto& rootward_tip : request.rootward_tips_) {
    ...
    quartet_root_plv_ = transition_matrix_ * GetPLV(rootward_tip.plv_idx_);
    for (const auto& sister_tip : request.sister_tips_) {
      ...
      quartet_r_s_plv_.array() =
          ...
          (transition_matrix_ * GetPLV(sister_tip.plv_idx_)).array();
      ...
      for (const auto& rotated_tip : request.rotated_tips_) {
        ...
        quartet_r_sorted_plv_.array() =
            ...
            (transition_matrix_ * GetPLV(rotated_tip.plv_idx_)).array();
