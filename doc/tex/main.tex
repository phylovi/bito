...
\begin{align*}
    ...
    &= \sum\limits_{s'|t' \in \tau^j} 1[t \in \tau^j] \left(1[s|t \in \tau^{j}] - \frac{\exp(\phi_{s|t})}{\sum\limits_{s'' \in \mathbb{C}_{.|t}} \exp(\phi_{s''|t})} \right) \\
    &= \sum\limits_{s'|t' \in \tau^j} 1[t \in \tau^j] \left(1[s|t \in \tau^{j}] - P_{\phi}(s|t) \right).
\end{align*}
...
\begin{align*}
    ...
    %EM I just realized this isn't quite right yet, is it? \mathbb{C}_{r} is the complete possible rootsplit support, while what we want here is the rootsplits of a specific unrooted topology.
    %EM Shall we use \mathbb{C}_r (\tau^j)?
...
\begin{align*}
    ...
    &= \sum\limits_{\rho \in \mathbb{C}_{r}} 1[t \in \tau_{\rho}^j] \left( P_{\phi}(\rho) \prod\limits_{s'|t' \in \tau_{\rho}^j : t' \not= t } P_{\phi}(s' | t') \right) \nabla_{s|t} P_{\phi}(s^*|t) \\
    &= \sum\limits_{\rho \in \mathbb{C}_{r}} 1[t \in \tau_{\rho}^j] \frac{P_\phi(\tau_{\rho})}{P_{\phi}(s^*|t)} \nabla_{s|t} P_{\phi}(s^*|t).
\end{align*}
...
\begin{align*}
    \nabla_{s|t} \log q_{\phi}(\tau^j)
    &= \frac{1}{q_{\phi}(\tau^j)} \sum\limits_{\rho} 1[t \in \tau_{\rho}^j] \times P_{\phi}(\tau^j_{\rho}) \times \left(1[s|t \in \tau^j_{\rho}] - P_{\phi}(s|t)\right),
\end{align*}
