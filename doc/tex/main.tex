...
%EM I guess we don't introduce \mathbb{C}_{.|t} really, do we?
%SJ They are now defined at the very beginning of this document.
...
%EM Perhaps this will become clear when we write out the VIMCO gradient, but somehow it would be nice to indicate where the "naive" assumption is applied.
%SJ It's naive compared VIMCO but otherwise, it's just a regular gradient estimator. Let's remove "Naive" if it leaves any room for confusion.

%EM I was using bold theta. We can discuss dropping it, or adding a macro, but let's make things consistent.
%SJ Introduced new \bParam macro.
...
%EM We should use different indices for the top and the bottom. I'd suggest j for the top and i for the bottom as above, but we could flip that if you feel strongly.
%SJ Done.
\begin{align}
    ...
    %EM Is there a reason you don't want to use the weights w^j?
    %SJ Nope. Just forgot to use it.
...
%EM I'd suggest replacing $.|t$ with $t$ in the following sentence. There's a little inconsistency here.
%SJ Done.
...
\begin{align*}
    ...
    %EM Don't we want to indicate that we are using a specific rooting in \prod\limits_{s'|t' \in \tau^i}? Something like \prod\limits_{s'|t' \in \tau^i_{r'}}?
    %SJ Agreed. Combined with comment below, I'm using \rho instead of r'.
...
\begin{align*}
%EM I found r' \in \mathbb{C}_{r} a little confusing, because "r" here is just an indicator for rooted, and "r'" is essentially an index. I propose two possible solutions:
%EM 1. use rho in place of r'
%EM 2. replace \mathbb{C}_{r} with \mathbb{R} and r' by r.
%SJ Going with the first solution.
...
%EM We're abbreviating with \hat{L}^K here-- shall we do that earlier?
%SJ I wanted to keep the parameters around above since we are taking gradient wrt them.
...
% \[
% \nabla_{{\bm{\phi}}}L^k({\bm{\phi}},{\bm{\psi}}) = \mathbb{E}_{q_{{\bm{\phi}},{\bm{\psi}}}(\tau^{1:K},\; {\bm{\bParam}}^{1:K})} \sum_{j=1}^K \left(\hat{L}^K({\bm{\phi}},{\bm{\psi}}) - \tilde{w}^j\right) \nabla_{\bm{\phi}} \log q_{\bm{\phi}}(\tau^j).
% \]
