...
Now, $q_{\bm{\phi}}(\tau^j)$ is going to be the sum over rootings of the rooted SBN probability of $\tau^j$: the probability of that rootsplit times the probability of all of the SBN conditional probabilities for that rooting.
Any SBN conditional probability will appear at most once in this sum.
We calculate
\[
    \nabla_{\bm \phi} \log q_{\bm \phi}(\tau^j) = \frac{\nabla_{\bm \phi} \, q_{\bm{\phi}}(\tau^j)} {q_{\bm{\phi}}(\tau^j)}
\]
by taking $\nabla_{\bm \phi} \, q_{\bm{\phi}}(\tau^j)$, which is a vector where the $\phi_i$th entry is the sum of $x / \phi_i$ for all terms $x$ of the summation that contain $\phi_i$.
Michael note:
we are going to be parameterizing $\phi_i$ as unnormalized parameters, therefore if we are taking a derivative with respect to a given child we need to have a nonzero term for every parent that goes with that child.
