...
\begin{enumerate}
    ...
    \item a means of mapping a branch $b$ to the variables $\psi_\varsigma$ involved in its parametrization
    \item a means of taking the relevant derivatives
\end{enumerate}

We could develop some new notation, but we might as well just look at \eqref{eq:PSPgradComponent}, and replace $b \psp \tau$ in our minds with ``the variables $\varsigma$ relevant for branch $b$ of topology $\tau$.''
...
\]
for a constant $c$.
Taking $g(\epsilon; \psi_\varsigma) := \exp(\mu_\varsigma + \sigma_\varsigma \epsilon)$,
\begin{align*}
...
The lognormal PSP parameterization uses the sum of the parameters across subsplits of the PSP:
\[
...
\gPSP(\theta; \bm\psi_{b \psp \tau}) :=
    ...
    \right)
\]
The derivatives are just like the above, with the only ``tricky'' part being
\[
    \frac
...
\]
when $\varsigma \in b \psp \tau$ (and zero otherwise), which is clearly different than $\frac{1}{\sigma_\varsigma}$ as for the simpler split-based distribution above.
